...
- base **is** `origin/main` → `gate ok: 15 of 15 stages ran (base <sha>)`,
...
```
gate ok: 15 of 15 stages ran
```

followed by the base it was a statement about. Both halves of the count matter. `15` is asserted against `EXPECTED_STAGES` in

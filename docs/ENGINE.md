...
- `profanity`'s capability set is `[xml, streams]`, which is the row this
whole section is actually about - and loses only `mono`/`room_window`,
which gate a single cosmetic feature (a duplicate exits window) this app
never reads. See `src-tauri/src/lich.rs`'s module doc for the full
verification. The registered-frontend path above is still the honest end
state; it is just not what declaring `--stormfront` gets today.

...
- [x] **R1  `buffs`** (≈70)
  commit: (this PR) verified: 2026-09-10 minutes: 60
  touches: lich-scripts/companion_bridge.lic, lich-scripts/test/protocol_harness.rb, src/bridge/mockBridge.ts, docs/BRIDGE_CONTRACT.md, docs/TESTING.md, tools/doc-claims-break-check.mjs
  depends-on: R0
  do: first because `NEXT-50.md:441-447` (#42) nominates it — read-mostly, so the blast radius of getting it wrong is small. Start the player's own buff script; report which buffs are up as progress. Do not invent a spell list: gap row 20 records that nothing in this repo knows DR's spells, and `LIVE-STATE.md:281` records the same decision being taken deliberately for Bard songs. `docs/TESTING.md` quotes the real implemented-intent count and the unimplemented-intent list (`test:doc-claims`); both drift the moment a HANDLERS entry lands, so this increment corrects both in the same commit rather than leaving the suite red for the next one to find. `tools/doc-claims-break-check.mjs` hardcoded the old count ("26") in its own sabotage fixture and aborted rather than silently passing once the doc said 27 — updated in the same commit, per its own stated rule that a fragment not found must abort, never pass quietly.
  verify: `node tools/intent-drift-test.mjs` shows 27 implemented, and the mock's unimplemented set drops to 8 in the same commit — confirmed: `node tools/intent-drift-test.mjs` reports "Implemented in bridge (real): 27", "Mock's claimed-unimplemented set: 8", "OK — mockBridge.ts matches the real bridge dispatch exactly." `node tools/activity-intent-contract-test.mjs` reports "9 of 9" with the `buffs` row now "implemented". Manually exercised against the real bridge via `lich-scripts/test/protocol_harness.rb` (Ruby 4.0.6 from `C:\Ruby4Lich5`): an independent WebSocket client sent `{type:'intent',intent:'buffs'}` and received `log "buff started"`, `scripts` listing `buff` as running, and `intent_ack ok=true`; a second call while running correctly refused `"already buffing (buff is running) - stop it, or wait for it to finish, first"`.
  sabotage: removed the `'buffs' => ...` line from `HANDLERS` entirely (a commented-out line was tried first and did not work — the JS extractor's regex matches inside Ruby comments too, so a `#`-commented entry still counted as implemented; real deletion was needed). Re-ran `node tools/intent-drift-test.mjs` (FAILED, named `buffs` as the mock overstating reality) and `node tools/activity-intent-contract-test.mjs` (FAILED, named the same). Also drove the sabotaged bridge directly: `hello.implementedIntents` no longer listed `buffs`, and sending the intent got the generic bridge-level refusal `"'buffs' is not implemented in bridge v0.14.0 yet."` — the exact mechanism `isIntentImplemented` reads to grey out a control, proven correct with no dedicated UI component in scope for R1 (none exists yet; R2/R4/R6/R7 are the increments that add per-intent controls). Restored, `node tools/intent-drift-test.mjs` and `node tools/activity-intent-contract-test.mjs` both green again, `ruby -c` syntax-checked.

- [x] **R2  `loot`** (≈60)
  commit: (this PR) verified: 2026-09-10 minutes: 75
  touches: lich-scripts/companion_bridge.lic, src/components/shared/InventoryPanel.tsx, src/bridge/mockBridge.ts, docs/BRIDGE_CONTRACT.md, docs/TESTING.md, tools/doc-claims-break-check.mjs
  depends-on: R0
  do: the Loot control is already in `InventoryPanel.tsx` and already gated on `isIntentImplemented(bridgeIntents, 'loot')`, so this is the bridge half only. `NEXT-50.md:449-456` (#43) is the specification. The batch contract (below) had flagged `loot` as unresolved — no standalone curated script found, `get all` was guessed as a possible existing quick-action. Neither guess held up: `get all` turns out to name nothing in this repo (only `src/data/macros.ts`'s `Take all` macro step, which sends the raw floor-pickup command, not a corpse loot), and the vendored scripts at `C:\Ruby4Lich5\Lich5\scripts` (read, not run) show the real DR verb dr-scripts automation actually sends is `loot <type>` (`stabbity.lic`'s `loot_mob`, `profiles/base.yaml`'s `custom_loot_type` comment: "treasure/equipment/all? What LOOT type to use for mobs"). So `Intents.loot` composes that one game command via `Cmd.exec` — the `stow_all`/`escape` shape, not `Script.start` — reading the character's own `custom_loot_type` setting the same way `buffs` reads `waggle_sets`, falling back to plain `all` only when neither an explicit arg nor a configured default exists. `docs/TESTING.md` and its own break-check fixture (`tools/doc-claims-break-check.mjs`) needed the same 27→28-implemented, 8→7-unimplemented correction R1 made for itself, in this commit.
  verify: `node tools/intent-drift-test.mjs` reports "Implemented in bridge (real): 28", true-unimplemented set 7, mock matches exactly. `node tools/activity-intent-contract-test.mjs` reports "9 of 9", `loot` row now "implemented". `ruby -c lich-scripts/companion_bridge.lic` syntax OK. Not live-tested against a real DragonRealms character (none available this session) — instead verified with an ad-hoc Ruby harness script built on the same stub pattern `lich-scripts/test/pause_test.rb` uses (loads `Companion` from the real file, stubs `Cmd.exec`/`State`/`Yaml`): 11/11 checks passed — Stop-latched refusal sends nothing; Pause-latched refusal sends nothing (generic fallback message, see below); no-arg/no-setting defaults to `loot all`; a configured `custom_loot_type` is read and used; an explicit `type` arg overrides the configured default; a `type` containing control/injection characters is rejected before anything is sent; a nil reply from `Cmd.exec` is reported honestly as "no response seen" rather than claimed as success. `docs/BRIDGE_CONTRACT.md`'s `loot` entry records this pattern is a best-effort guess at the game's real reply text, not a captured transcript, and flags it for a future live-tested tightening pass. With the handler present the control enables (`isIntentImplemented` reads real `HANDLERS.keys`, unaffected by this change other than gaining the key); with it absent (a sabotaged tree) the control is disabled — same mechanism R1 proved for `buffs`, not re-proven here. `node tools/run-tests.mjs`: 176 passed, 0 failed, 7455 checks (5 pre-existing partial suites, all Rust/Godot-content gaps unrelated to this increment). `npm run gate` (`DRC_TEST_PORT=48812`): `gate: 16 of 16 stages ran in 843s, no failures — but 10 thing(s) went unchecked` in the same 5 pre-existing suites, base `6ba009ae` = `origin/main` (current).
  sabotage: not run separately for this increment — `tools/intent-drift-test.mjs` and `tools/activity-intent-contract-test.mjs` already have their own sabotage coverage (proved by R0/R1) for "a HANDLERS entry disappears while the doc still claims it", which is the class this increment's own risk belongs to; no new checking mechanism was added here to need a fresh one.
  note: one deliberate departure from R0's stated mechanism, recorded in `companion_bridge.lic` next to `PAUSE_HELD` and in `BRIDGE_CONTRACT.md`: `loot` calls `pause_refusal('loot')` but has no line in `PAUSE_HELD`, so a paused player reading the refusal gets the shared "Paused - press Resume." sentence, not a bespoke one. `tools/pause-reaches-travel-test.mjs`'s "movers" population is Script.start plus a named `PLAYER_SUPPLIED_COMMANDS` list (run_macro) by explicit design ("The second cannot be derived from Script.start, so it is named... rather than assumed") — `loot` is neither, so a `PAUSE_HELD` entry for it is a stray the test flags, matching the precedent already set by `stow_all`/`escape`/`armor_manage`/`check_health`/`check_toggles`, none of which has one either despite all five sending real `Cmd.exec` commands. Pause safety itself is not weakened by this — `pause_refusal` still refuses the intent — only the wording is generic. Flagged rather than silently worked around: a future session extending that test's classifier to cover this whole Cmd.exec-composed category (and auditing whether the other five should also gain their own sentences) is real, separate work, not a fix folded into this touches list.

- [x] **R3  `travel`** (≈90)
  commit: (this PR) verified: 2026-09-10 minutes: 85
  touches: lich-scripts/companion_bridge.lic, src/bridge/mockBridge.ts, src/data/travelPath.ts, docs/BRIDGE_CONTRACT.md, docs/TESTING.md, tools/doc-claims-break-check.mjs
  depends-on: R0
  do: `map_walk` already exists and already starts `;go2` rather than pathfinding itself — copy that shape exactly; `DOMAIN.md:1059-1060` is the argument for it ("A companion that drives `;go2` inherits every fix anyone makes to it"). What `travel` adds over `map_walk` is a *named destination* rather than a room id, and the two hard parts are both recorded: `DOMAIN.md:694-696`, arrival must be **checked** after the move completes and never inferred; and `:96-101`, an expired passport strands a character, so passport state is runtime state the bridge must report before a leg is attempted. Implemented: `Intents.travel` in `companion_bridge.lic` passes `args['destination']` straight to `Script.start('go2', destination)`, matching `map_walk`'s refusal order (Stop, Pause, `go2` not installed, already traveling) with no path precheck — a tag/uid destination cannot be `path_to`-resolved without reimplementing `go2`'s own resolution, which R0's rule forbids. Arrival is confirmed, not inferred: `watch_travel_arrival` polls in its own thread (bounded 20 minutes) until `go2` stops running, then `report_travel_arrival` compares the real room Lich reports (numeric id, `u<uid>`, or the room's own tags for a tag destination) against the destination and logs the true outcome — including "cannot be confirmed" when `MapInfo.available?` is false or `MapInfo.current` is nil, the `LIVE-STATE.md:320` no-map-database case, rather than ever reading "go2 exited" as "arrived" (the exact `install_mapdb` history bug this increment's own comment cites). Passport validity has no live Lich read at all, so rather than a check this bridge cannot make, an F2P/unverified-tier account gets a `log` warning before the leg starts, built from `State.account_tier` (already published as `accountTier` on every status — no new `CharacterStatus` field added, per the touches list). `src/bridge/mockBridge.ts`'s `travel` case now mirrors this and `map_walk`'s mock (no real Lich in demo mode, say so) instead of the old `planTravel`/`travelPath.ts` step-by-step route simulator, which reimplemented client-side exactly the pathfinding `DOMAIN.md:1059-1060` warns against; that simulator is deleted from `travelPath.ts`, `listReachable` (a menu filter, not a router, still used by `ScriptLauncher.tsx`) is unchanged.
  verify: `node tools/intent-drift-test.mjs` reports "Implemented in bridge (real): 29", true-unimplemented set 6, mock matches exactly ("OK — mockBridge.ts matches the real bridge dispatch exactly"). `node tools/activity-intent-contract-test.mjs` reports "9 of 9", `travel` row now "implemented", no warnings. `ruby -c lich-scripts/companion_bridge.lic`: Syntax OK. `node tools/pause-reaches-travel-test.mjs`: `travel` reads as a mover (calls `Script.start`), is declared in `PAUSE_HELD`, consults `pause_refusal('travel')` by name — 99 checks, all passed. Not live-tested against a real DragonRealms character (none available this session, same as R2) — the honest-arrival and no-map-database paths were verified by code inspection against `MapInfo`'s real behaviour (`available?` is false when `Lich::Common::Map`/`Map` is not loaded at all; `current` returns nil when no rooms are loaded — the no-map-database case — both handled explicitly and neither ever read as arrival) rather than a live or stubbed Ruby harness, which is the gap flagged below. `npx tsc -b`: clean. `node tools/doc-claims-break-check.mjs`: 22 sabotages across 14 files, 0 did not redden exactly the checks they named, including the two edited `docs/TESTING.md` cases (implemented-count 29→2, unimplemented-list fragment). `node tools/run-tests.mjs`: 176 passed, 0 failed, 7458 checks (5 pre-existing partial suites — Godot 2D content-pack gaps and Rust-only e2e checks — unrelated to this increment, same set R2 recorded). `npm run gate`: `gate: 16 of 16 stages ran in 798s, no failures — but 10 thing(s) went unchecked` in the same 5 pre-existing suites, base `e6a1c870` = `origin/main` (current). The no-map-database pitfall below was established by code reading, not a live run — `MapInfo.available?`/`.current` are exercised the same way `map_walk`'s own `no_map` guard already is, and this increment's watcher adds no new failure mode there.
  sabotage: not run separately for this increment, same reasoning R2 recorded — `tools/pause-reaches-travel-test.mjs` already sabotage-covers "a mover skips its latch check" (proved by #462/map_walk), and `tools/intent-drift-test.mjs`/`tools/activity-intent-contract-test.mjs` already sabotage-cover "a HANDLERS entry disappears while the doc still claims it" (proved by R0/R1); this increment's own new surface — the arrival watcher never claiming success without checking the room — has no dedicated check yet to sabotage, because nothing else in this repo asserts it structurally; a future increment adding a Ruby-level `travel`/arrival test (the gap the verify line above names) is the place a sabotage for this specific class belongs, not folded into this touches list.
  pitfalls: `LIVE-STATE.md:320` — on the one live run Lich had no map database at all, so every map intent answered nothing. Handled: `report_travel_arrival` checks `MapInfo.available?` and `MapInfo.current` explicitly and reports "cannot be confirmed" in both no-map-database shapes rather than silently reading either as success or crashing — established by code reading against `MapInfo`'s definitions, not a live run (none available this session).

- [x] **R4  `escape_heal` and `go_healer`** (≈80)
  commit: (this PR) verified: 2026-09-10 minutes: 110
  touches: lich-scripts/companion_bridge.lic, src/components/layout/SituationBanner.tsx, src/bridge/mockBridge.ts, docs/BRIDGE_CONTRACT.md, docs/TESTING.md, tools/doc-claims-break-check.mjs
  depends-on: R3
  do: `NEXT-50.md:475-482` (#46) specifies both. Two constraints from the domain, both of which have burned somebody already: `DOMAIN.md:669` and `:829` — **the threshold that decides "go heal" belongs to the player and must be visible**, not buried; two separate players hit this confusion in two separate channels, which `DOMAIN.md:826` calls a design problem rather than user error. And `:481-485` — a player picks a **home healer** and explicitly rejects proximity, so a preferred heal city wins by default and `map_nearest` is the fallback, not the rule. Implemented: `go_healer` resolves `args['preferredCity']` (already sent by `bridgeIntentDispatcher.ts`, sourced from the Settings sheet's existing heal-city picker — no new client wiring needed) against dr-scripts' own town data (`get_data('town')[town]['npc_empath']['id']`, the field `safe-room.lic` already reads) to get a room; no city set, an unrecognised one, or one with no known healer room all fall back to `nearest('npchealer', 1)` — `map_nearest`'s own read-only lookup — logged every time it happens, never silent. The client-side `chooseHealer`/`scoreHealers` scoring in `src/data/healers.ts` is deliberately not reimplemented in Ruby, per `docs/BRIDGE_CONTRACT.md`'s own research for this intent. `escape_heal` composes `escape` (already shipped) then `go_healer`, reusing both rather than a third implementation, and logs each as its own state (`NEXT-50.md`'s "combat escape and travel report separate states"). The low-health banner's own trigger (`LOW_HEALTH_SHARE` in `src/lib/vitals.ts`, already the single source both `SafetyFooter.tsx` and this banner read) is now stated in `SituationBanner.tsx`'s text ("34 of 118 health — below 35%") rather than left for the player to infer — the fix `DOMAIN.md:669,:829` calls for, scoped to this increment's one touched file.
  verify: no "healed" result appears before live health confirms it — `watch_healer_arrival` (built on R3's `wait_for_script_to_stop`/`report_travel_arrival`, both pulled out of `watch_travel_arrival` for this reuse) never logs a "healed" outcome until `go2` has stopped, arrival at the resolved room is confirmed against the real room Lich reports, and `read_health_result`/`wound_summary` (both pulled out of `check_health` for this reuse) confirm zero wounds/bleeding/poison/disease from a fresh `DRCH.check_health` call. `ruby -c lich-scripts/companion_bridge.lic`: Syntax OK. `node tools/intent-drift-test.mjs`: "Implemented in bridge (real): 31", true-unimplemented set 4, "OK — mockBridge.ts matches the real bridge dispatch exactly." `node tools/activity-intent-contract-test.mjs`: "9 of 9", both rows now "implemented". `node --experimental-test-module-mocks tools/pause-reaches-travel-test.mjs`: 101 checks passed; `go_healer` is derived as a mover (calls `Script.start`), is declared in `PAUSE_HELD`, and consults `pause_refusal('go_healer')` by name; `escape_heal` correctly does *not* appear in `PAUSE_HELD` (its own method body never calls `Script.start` directly — it delegates to `go_healer`, which does — so the test's static mover-derivation does not expect a line there, and adding one would have failed the "no stray entry" check). `npx tsc -b`: clean. `node tools/doc-claims-test.mjs`: 101 checked, 0 failed, including "TESTING quotes the real implemented-intent count (doc 31, bridge 31)" and "TESTING names as many unimplemented intents as there are (doc names 4, drift test counts 4)". `node tools/run-tests.mjs`: 175 passed, 1 failed, 5 partial, 7461 checks across 176 suites — the 1 failure (`test:ai-script-repair`) and the 5 partials (Godot 2D content-pack gaps) are pre-existing and unrelated to this increment's files, confirmed by running the exact same baseline (174 passed, 2 failed, same 5 partial) before any R4 edit landed — the second pre-existing failure (`test:play-first-layout-break`) is flaky and passed on the post-edit run. `npm run gate`: `gate: 16 of 16 stages ran in 686s (base 2619dd5e = origin/main at claim time), failed: suites, break-ai-script-repair` — both failures are the same pre-existing `test:ai-script-repair` gap, not caused by this increment. A failure leaves a stated recovery action: `go_healer`'s refusals (no `go2`, already traveling, no heal city and no map) each name what to do next; the arrival-watcher's two failure branches ("could not confirm arrival ... send Go to Healer again once you know where you are" / "N wound(s) remain ... send Go to Healer again, or ask a nearby empath directly") and the not-installed/no-heal-city refusal all do the same.
  sabotage: not run separately for this increment, same reasoning R2/R3 recorded — `tools/intent-drift-test.mjs`/`tools/activity-intent-contract-test.mjs` already sabotage-cover "a HANDLERS entry disappears while the doc still claims it" (proved by R0/R1) and `tools/pause-reaches-travel-test.mjs` already sabotage-covers "a mover skips its latch check" (proved by #462/map_walk); this increment's own new surface — the healer-outcome watcher never claiming "healed" without a live check — has no dedicated sabotage-covered guard yet, the same gap R3 flagged for its own arrival watcher.
  pitfalls: not live-tested against a real DragonRealms character (none available this session, same gap R1–R3 recorded) — the arrival/healing confirmation logic was verified by code reading against `MapInfo`'s and `DRCH.check_health`'s real return shapes (unchanged from R3/`check_health`'s own verified behaviour), and `get_data('town')`'s field structure was verified by reading the installed `C:\Ruby4Lich5\Lich5\scripts\data\base-town.yaml` and `safe-room.lic` directly rather than assumed.
...
- [x] **W2  Container capacity is hardcoded zero** (≈60)
  commit: (this PR) verified: 2026-09-10 minutes: 105
  touches: lich-scripts/companion_bridge.lic, src/components/shared/InventoryPanel.tsx, src/types/index.ts, src/lib/versions.ts, docs/BRIDGE_CONTRACT.md, ruby/lich_stub.rb, lich-scripts/test/protocol_harness.rb, lich-scripts/test/server_test.rb, new:lich-scripts/test/container_test.rb, package.json, tools/test-suites.json
  depends-on: W0
  do: the bridge emitted `{'name' => c, 'used' => 0, 'capacity' => 0}` for every worn container. The fields exist, cross the socket, and are a constant — a value that looks like an answer and is a placeholder, which is worse than the absence. `NEXT-50.md:394-400` (#37): "Capacity and location matter more than a long alphabetical list." **The bug was one layer worse than the line the plan cited**, exactly as `docs/BRIDGE_CONTRACT.md`'s container contents contract predicted: `DRCI.get_worn_containers` exists nowhere in Lich or dr-scripts — `grep -rn get_worn_containers /c/Ruby4Lich5/Lich5` returns only this script and its own `.bak`, while the same grep for `open_container?` finds a real definition — so it raised `NoMethodError` into `safe([])` on every tick and a live bridge reported *no containers at all*. Implemented against `GameObj.containers`, Lich's own `{container id => contents}` registry, filled passively from the game's `<inv id='...'>` blocks and committed at each prompt (`lib/common/xmlparser.rb`, `lib/common/gameobj.rb`). That costs **no game command**, which retires the contract's own deferral — it had parked this behind an on-demand intent because rummaging every bag on a status tick means roundtime, message-log noise and opening something a player may be mid-use of. A container Lich has no entry for is **omitted**, not zeroed, so `InventorySummary.used` became optional: absent is uncounted, `0` is empty. `capacity` stays `0` and no qualitative field was invented in its place, because DR exposes neither — checked, not assumed: the only numeric capacity read in the whole tree is `DRCI.count_lockpick_container` (`APPRAISE <ring> QUICK`, lockpick rings only), and an ordinary container's sole fullness signal is the failure text when a PUT does not fit (`PUT_AWAY_ITEM_FAILURE_PATTERNS`), which is a consequence of trying rather than a value that can be read. `BRIDGE_VERSION`/`EXPECTED_BRIDGE_VERSION` 0.14.0 → 0.15.0 together (`tools/bridge-version-drift-test.mjs` requires both), so a stale bridge still sending the old constant `0` is named by `compareVersions` rather than silently read as a real empty — that is why `src/lib/versions.ts` is in the touches list. Two smaller defects fixed in passing: `reset_degraded!` ran *after* the container read, so a swallowed read of this very field could never be reported as degraded; and `ruby/lich_stub.rb` plus `lich-scripts/test/protocol_harness.rb` both stubbed `get_worn_containers`, which is precisely why no test could see that the real method was missing — both removed, `GameObj.containers` added in their place.
  verify: a full bag and an empty bag read differently. That sentence is the whole check and it is one the current code cannot pass. Built as `lich-scripts/test/container_test.rb` (`npm run test:containers`, registered in both `package.json` and `tools/test-suites.json` per trap 2), whose fixture deliberately carries a three-item bag, an empty bag, an uncounted bag and a non-container at once — a discriminator checked against a population where the wrong answer is available. 17 checked, 0 failed, including `full=3 empty=0` and a denominator guard that fails if fewer than 15 checks ran. `npm run test:server` 175 checked, 0 failed, now asserting the same distinction end-to-end through the socket and that a broken `GameObj` names `containers` among the degraded fields. `node tools/ai-script-repair-test.mjs` standalone: 149 passed, 0 failed, 148 checks. `npx tsc -b` clean. `node tools/run-tests.mjs`: 177 passed, 0 failed, 7482 checks across 177 suites (baseline before any edit: 176 passed, 0 failed, 7462 — so the total rose by exactly the new work), same 5 pre-existing partials R2–R4 recorded. **Not exercised against a live DragonRealms character** — none available this session, the same gap W1 and R1–R4 recorded; the contract's acceptance check (wear two containers with known different counts) is still outstanding.
  pitfalls: **`src/bridge/mockBridge.ts` cannot reach either of the two states a real bridge now produces**, and that is left standing rather than fixed here. Every one of its five container fixtures carries `capacity` above zero, so demo mode always draws the used/capacity bar — the one branch a live bridge can never take, since DR exposes no capacity. Nobody developing against the mock will see `N items` or `contents unknown`, which is the whole visible surface of this increment, and that is the "a state the fixture cannot reach is a state nobody sees until a user does" trap in its exact form. Not folded in: the mock was outside this increment's `touches` and the gate above was already running against the tree when this was found, so changing it would have shipped a file the gate did not cover. It is one fixture edit for whoever takes W4, which touches `InventoryPanel.tsx` again. **Not exercised against a live DragonRealms character**, as the verify line records; the contract's acceptance check is the outstanding half.
  sabotage: the pre-W2 body was restored verbatim — constant `'used' => 0, 'capacity' => 0` over `GameObj.inv`, deliberately fed by a call that *resolves* here so the sabotage reaches the assertion instead of being intercepted by the `NoMethodError` that hid the original bug. It reddened exactly four named checks and no others, the decisive one reading `full=0 empty=0`. Restored from a byte copy, `md5sum -c` OK, `ruby -c` Syntax OK, suite green again. Separately, `containers` was removed from `ruby/lich_stub.rb`'s `SURFACE` list: `tools/ai-script-repair-test.mjs` went red naming `74 calls found by Ruby's parser, 1 uncovered: GameObj.containers`, proving that shim check actually covers the new API rather than being assumed to; restored, `md5sum -c` OK.
...
- [x] **W4  Encumbrance as numbers** (≈45)
  commit: (this PR) verified: 2026-09-10 minutes: 115
  touches: lich-scripts/companion_bridge.lic, src/types/index.ts, src/components/shared/InventoryPanel.tsx, src/lib/versions.ts, src/lib/panelDataContracts.ts, src/bridge/mockBridge.ts, docs/BRIDGE_CONTRACT.md, ruby/lich_stub.rb, lich-scripts/test/protocol_harness.rb, lich-scripts/test/server_test.rb, tools/character-status-fields-test.mjs, new:lich-scripts/test/encumbrance_test.rb, package.json, tools/test-suites.json
  depends-on: W0, W2
  do: `encumbrance` arrives as a string. `DOMAIN.md:142` gives the real shape — 100 items free-to-play, 75 before junk-room warnings, 300/250 with the inventory upgrade, and slot limits of body 10, over shoulder 2, finger 2, belt 2 — and `:210-212` explains why it is not cosmetic: burden and armour reduce effective Athletics, which is what decides whether a shortcut kills you. **Both halves of the answer turned out to be readable, and neither was where the spec looked.** The word *does* have a number behind it: `Lich::DragonRealms::ENC_MAP` (`lib/dragonrealms/drinfomon/drvariables.rb`) is DragonRealms' own twelve-phrase ladder, `None`→0 .. `It's amazing you aren't squashed!`→11, with `DRC.check_encumbrance` as the in-tree caller — so `encumbranceLevel`/`encumbranceScaleMax` read Lich's ladder at runtime rather than copying it, exactly as `balance`/`position` are sent raw, and no second copy of the ordering exists in this repository. A phrase **not** on the ladder sends `null`, never `0`, because `0` means unburdened and those are opposite answers to "can I pick this up". The item count has no call at all: `DRCI.count_items`/`count_items_in_container`/`count_lockpick_container` are the only counting methods in the whole tree and every one of them issues RUMMAGE, TAP or APPRAISE — roundtime and message-log noise on a status tick, the same objection W2's contract raised. DR publishes no running total; the item limit is visible **only** as a refusal ("would push you over the item limit", "carrying far too many items" — `commons/common-items.rb:156,1857`, `scripts/combat-trainer.lic:736,1361`), which is a consequence of trying, precisely like container capacity. So `carriedItemCount` is derived, from the three places Lich itself enumerates when asking whether an object is still on the character (`lib/gemstone/readylist.rb:63`): `GameObj.inv`, the contents of the carried containers in `GameObj.containers` (which W2 had just wired), and the two hands. Container contents are in the sum because STOW — which puts an item *into* a container — is one of the commands that can be refused for the limit. **It is therefore a floor and is rendered as one**: a bag nobody has opened contributes nothing, the same absence `InventorySummary.used` already carries, and "at least N" is the only honest form. Only *carried* containers are summed; a chest standing in a room can leave contents in the same registry. Two things ruled out rather than assumed: `XMLData.encumbrance_value` is a real numeric percentage and is **GemStone-only** — `global_defs.rb:2319` gates the whole `encumlevel`/`pbarStance`/hands block behind `XMLData.game.to_s.match?(/GS/)`, which `src/types/stream.ts` had already recorded and which was re-checked here rather than taken on trust; and `GameObj#type` cannot classify a worn thing as a container on this machine at all, because `data/gameobj-data.xml` does not exist (`find /c/Ruby4Lich5 -iname 'gameobj-data*'` → nothing, against a control that finds `drvariables.rb` twice), so no field distinguishing "unopened container" from "shirt" was invented. `BRIDGE_VERSION`/`EXPECTED_BRIDGE_VERSION` 0.15.0 → 0.16.0 together (`tools/bridge-version-drift-test.mjs` requires both), so a bridge that never sends these is named as stale rather than read as a character carrying nothing — there is deliberately no client-side fallback to `wornCount + looseCount`, which is the undercount this replaces. Three defects fixed in passing, all of them the same shape: `InventoryPanel` coloured the readout by matching `encumbrance` against `'overloaded' | 'heavy' | 'moderate' | 'light'`, **none of which is a DragonRealms phrase**, so on a live bridge every comparison was false and "Tottering Under Burden" drew in the same green as "None"; the existing carry warning counted `wornCount + looseCount`, so a character with eleven worn things and ninety items in bags read as eleven and was never warned; and `protocol_harness.rb` reported `'Light'`, a word that is on no ladder, so the harness could only ever have exercised the "we could not tell" branch. Also closes **#562**: every `capacity` in `src/bridge/mockBridge.ts` was above zero, so demo mode could only draw the capacity bar — the one branch a live bridge can never take — and the two states a real player sees (`N items`, `contents unknown`) were unreachable. All ten fixtures are now `capacity: 0`, four containers omit `used` so `contents unknown` and its "Scan this container in game" row are on screen, one is opened-and-empty so W2's distinction is visible too, and the mock's `carriedItemCount` is *derived from its own inventory fixture* rather than typed, so a fixture edit cannot leave the count disagreeing with the containers beside it.
  verify: the readout answers "can I pick this up", which a word cannot. Built as `lich-scripts/test/encumbrance_test.rb` (`npm run test:encumbrance`, registered in both `package.json` and `tools/test-suites.json` per trap 2), whose fixture makes every wrong answer a *different* number that the test names: 4 is forgetting containers and hands, 5 is forgetting container contents, 7 is forgetting the held item, 11 is summing a chest that is standing in a room, 8 is right — a discriminator checked against a population where the wrong answers are available. It also runs the same rank code over a phrase on the ladder and a phrase off it, because a rank is only worth having if an unrecognised word comes back `nil` rather than `0`. 21 checked, 0 failed, with a denominator guard that fails below 18. `npm run test:server` 179 checked, 0 failed (baseline 175), now asserting all three fields cross the socket with the count checkable against the same three worn things, one open bag and one occupied hand the neighbouring checks assert — and that a broken `GameObj` names `carriedItemCount` as degraded while `encumbranceLevel`, which reads DRStats and the ladder, correctly does **not**. `node tools/ai-script-repair-test.mjs` standalone: 149 passed, 0 failed, 76 calls found by Ruby's parser, 0 uncovered. `node tools/character-status-fields-test.mjs`: 23 checked, 0 failed, both new fields documented, registered in `PANEL_DATA_CONTRACTS` and read by `InventoryPanel.tsx`. `npx tsc -b` clean. `node tools/run-tests.mjs`: 178 passed, 0 failed, 7516 checks across 178 suites (baseline before any edit: 177 passed, 0 failed, 7482 across 177 — so the suite count rose by exactly the new suite), same 5 pre-existing partials. **Not exercised against a live DragonRealms character** — none available this session, the same gap W1, W2 and R1–R4 record — and, separately, **not seen rendered**: a preview server started for this worktree served `origin/main`'s `mockBridge.ts` (`capacity: 8` where disk says `capacity: 0`, confirmed by fetching the module the server was transforming), so the browser check was abandoned rather than reported on stale output. The panel change is covered by `tsc -b` and by the field-reader check, not by eye.
  sabotage: two, each restored from a byte copy. (1) `ruby/lich_stub.rb`'s `SURFACE` entry for `Lich::DragonRealms::ENC_MAP` removed → `tools/ai-script-repair-test.mjs` red naming `76 calls found by Ruby's parser, 2 uncovered: Lich::DragonRealms::ENC_MAP.[], Lich::DragonRealms::ENC_MAP.values`, proving that shim check actually covers the new call rather than being assumed to; restored, `md5sum -c` OK. The ladder is called on the constant path rather than through a local **because of** that check: `ruby/runner.rb --surface` records only a `CallNode` whose receiver is a constant, so binding it to a variable first would have made both calls invisible to the extractor and the shim could then have lost the entry with nothing going red. (2) The bridge reverted to the two answers this increment exists to replace — an unrankable phrase returning `0` instead of `nil`, and the container sum dropped from the count → `encumbrance_test.rb` red on exactly five named checks, the decisive ones reading `an unrecognised phrase does not rank: 0` and `it is 8 ...: got 5`, and `server_test.rb` red on exactly one, `carriedItemCount ...: 4`. No other check moved in either suite. Restored, `md5sum -c` OK, `ruby -c` Syntax OK, both green again. The first attempt at (2) **aborted rather than running**: its fragment was built with `\n` against a file whose region is CRLF-sensitive (trap 22), matched zero times, and the harness refused instead of rewriting the file unchanged and reporting a pass.

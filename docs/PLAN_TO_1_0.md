...
An empty table means every lane is free to claim. **N2 is done** — merged 6 Sep 2026, and no row was added for it: it was a one-PR increment, and a row whose only job is to stop a second session claiming N2 has nothing left to do the moment N2 is `[x]` (Lane H’s precedent, above). **N1 has no dependencies and no file overlap with anything**, so a session can start Lane N there right now; read

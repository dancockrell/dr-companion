...
Implemented in bridge (real):       27
True unimplemented set:              8
```

The 27 come from the `HANDLERS` hash in `companion_bridge.lic` — a runtime
...
- The eight intents `tools/intent-drift-test.mjs` reports as unimplemented, which
  at the time of writing are `burgle`, `escape_heal`, `go_healer`,
  `loot`, `start_combat`, `start_training`, `town_run` and `travel` — buffs
  landed R1, 10 Sep 2026. Run it rather than trusting that list.

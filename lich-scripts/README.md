...
| `escape` | Sends `flee`. The declared emergency exit; nothing composed, just DR's own escape |
| `run_macro` | Sends literal commands from a macro/task-flow step, args: `{ commands: string[] }`. Sequential, each waiting real roundtime — a Stop sent mid-run queues behind it rather than pre-empting it, same as `stow_all` |

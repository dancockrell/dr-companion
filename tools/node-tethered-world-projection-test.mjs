...
if (!existsSync(outputPath)) fail('the node-tethered projection is generated')
  ...
  if (projection.schemaVersion === 2 && projection.view?.projection === 'fixed-orthographic-isometric' && projection.view?.rotation === 'locked' && projection.animationPhase === 'static-rigged') pass('projection declares the fixed isometric, rigged-but-static presentation phase')
  else fail('projection retains obsolete free-camera or animation assumptions')
  if (projection.kitContract?.authority === 'mud-room-graph' && projection.kitContract.baseFamilies?.length === 3 && projection.kitContract.portableConcepts?.includes('typed-tethers')) pass('kit metadata is reusable while DR topology remains authoritative')
  else fail('kit portability or DR authority contract is missing')
  if (projection.transitions.every((transition) => nodeIds.has(transition.fromNodeId) && nodeIds.has(transition.toNodeId) && transition.presentation === 'confirm-then-snap' && transition.futureTraversalEffect === 'streak-along-tether')) pass('every transition snaps only after confirmation and reserves truthful streak traversal for later')

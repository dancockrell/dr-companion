...
const DESCRIBES = {
  ...
  'drc.scene-rail-width.v1': 'How wide the right rail is, as a fraction of the window. One key because the play-first frame has one divider: the text takes whatever the rail leaves. Replaces `drc.left-rail-width.v1`, `drc.board-slot-width.v1` and `drc.right-rail-width.v1`, which described three columns that no longer exist - bumped rather than reused, because a share that meant "13% for the character rail" read as this one would give the scene pane a quarter of the width it needs.',
  'drc.scene-pane.v1': 'Whether the scene pane is in the corner, in a window of its own, or hidden - one answer per class of window size, because it is a different decision on a 1997px monitor and on a 720px minimum window. See src/lib/scenePane.ts.',

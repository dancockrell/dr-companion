...
check('room exits stay pinned above a complete clickable floor rail', /min-h-0 flex-1[^"']*overflow-y-auto/.test(classicRoom) && /aria-label="Room exits"/.test(classicRoom) && /aria-label="Clickable room items"/.test(classicRoom) && classicRoom.indexOf('aria-label="Room exits"') < classicRoom.indexOf('<FloorItems items={items}'))

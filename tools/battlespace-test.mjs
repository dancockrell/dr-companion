...
check('every combat variation is a directly wired compact icon button', /macro\.variations\.map/.test(actions) && /onClick=\{\(\) => run\(variation\.commands\)\}/.test(actions) && /h-8 w-8/.test(actions))
check('game and multiple taskflows permanently share an equal overflow-safe workspace', /grid-cols-\[minmax\(0,1fr\)_minmax\(0,1fr\)\]/.test(chat) && /gap-1 p-1/.test(chat) && /Game and automation workspace/.test(chat) && /title="Scripts & tasks"/.test(chat) && /<StreamTabs/.test(chat) && /<TaskFlowPanel/.test(chat) && !/paneMode/.test(chat))
...
check('the scripts pane no longer duplicates basic commands - BattleActionBar owns those now', !/MACROS/.test(taskflows) && !/useMacroRunner/.test(taskflows) && !/runMacro\(variation\.commands\)/.test(taskflows) && !/command\.\$\{macro\.id\}/.test(taskflows) && /rubyScripts/.test(taskflows) && /catalogs\.scripts\.value/.test(taskflows) && /listScripts\(\)/.test(taskCatalogStatus))
...
check('the scripts and tasks launcher uses semantic icons, colors and fixed-square geometry', /accentForIndex\(entryVisualIndex/.test(taskflows) && /data-entry-id=\{entry\.id\}/.test(taskflows) && /grid-cols-\[repeat\(auto-fill,2rem\)\]/.test(taskflows) && /h-8 w-8/.test(taskflows))
check('unavailable command buttons are honestly disabled instead of accepting dead clicks', /disabled=\{!canSend\}/.test(actions))
check('battle commands can be added to and removed from the shared hotbar', /kind: 'command'/.test(quickSwitch) && /kind: 'command', actionKey/.test(actions) && /Remove .* from the hotbar/.test(hotbar) && /togglePin\(slot\.pin\)/.test(hotbar))
...
check('map operations and symbols form one grab-scrollable two-row fixed-square rail', /grid-flow-col grid-rows-2/.test(mapToolRail) && /drag\.onPointerMove/.test(mapToolRail) && /<PinPalette/.test(mapToolRail) && /h-8 w-8/.test(pinPalette) && /h-8 w-8/.test(mapPinBar) && /h-8 w-8/.test(quickTravel) && !/Compass/.test(quickTravel))
...
check('radar motion respects reduced-motion and every actor has a visible keyboard focus', /animate-pulse motion-reduce:animate-none/.test(radar) && /focus-visible:ring-2/.test(radar) && /motion-reduce:transition-none/.test(battle))

...
check('the functions pane launches every basic macro variation beside every discovered script', /MACROS\.flatMap/.test(taskflows) && /runMacro\(variation\.commands\)/.test(taskflows) && /command\.\$\{macro\.id\}/.test(taskflows) && /rubyScripts/.test(taskflows) && /catalogs\.scripts\.value/.test(taskflows) && /listScripts\(\)/.test(taskCatalogStatus))
...
check('the complete functions launcher reuses semantic icons, colors and fixed-square geometry', /actionKey: `\$\{macro\.id\}:\$\{variation\.id\}`/.test(taskflows) && /actionIcon\(entry\.actionKey\)/.test(taskflows) && /accentForIndex\(entryVisualIndex/.test(taskflows) && /data-entry-id=\{entry\.id\}/.test(taskflows) && /grid-cols-\[repeat\(auto-fill,2rem\)\]/.test(taskflows) && /h-8 w-8/.test(taskflows))

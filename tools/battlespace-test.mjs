...
check('crowded rooms summarize duplicate inert lists while preserving the interactive rails', /FULL_SUMMARY_LIMIT = 8/.test(classicRoom) && /loose items in/.test(classicRoom) && /searchable, clickable floor controls/.test(classicRoom) && /inspect the portrait rail/.test(classicRoom) && /slice\(0, 3\)/.test(classicRoom))
...
check('combat pressure yields width to Battle without erasing the saved Experience preference', /battleActive \? 220/.test(app) && /dashGrowthMax/.test(app) && /dashGrowthMax/.test(readFileSync(new URL('../src/lib/columns.ts', import.meta.url), 'utf8')))
...
check('the room header suppresses only the duplicate combat chip and truncates identity safely', /<StatusBoard hideInCombat/.test(battleStatus) && /hideInCombat && r\.flag === 'in_combat'/.test(statusBoard) && /max-w-36[^"']*truncate/.test(handsRow) && /title=\{who\}/.test(handsRow))

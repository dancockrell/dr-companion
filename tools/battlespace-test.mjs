...
check('the floor renders in exactly one place, not a glance strip plus a browser', !/footer=\{roomItems/.test(battle) && !/mode="glance"/.test(battle) && !/\bfooter\b/.test(scene) && !/--radar-loot-height/.test(scene) && !/export type FloorItemsMode/.test(floor) && !/mode\?:/.test(floor))
check('actor hover cards expose live facts, profile facts, game actions, and Elanthipedia', /playerProfileFor/.test(radar) && /label="Race"/.test(radar) && /label="Guild"/.test(radar) && /Right now/.test(radar) && /Bestiary/.test(radar) && /requestGameAction\(`look/.test(radar) && /requestGameAction\(`assess/.test(radar) && /Special:Search/.test(radar) && /pointer-events-auto/.test(radar))
check('the scripts pane no longer duplicates basic commands - BattleActionBar owns those now', !/MACROS/.test(taskflows) && !/useMacroRunner/.test(taskflows) && !/runMacro\(variation\.commands\)/.test(taskflows) && !/command\.\$\{macro\.id\}/.test(taskflows) && /rubyScripts/.test(taskflows))
...
check('radar rails are transparent textless overlays on the battle map', /bg-transparent/.test(radar) && !/Combat radar<\/span>/.test(radar) && !/tone="friendly"/.test(radar) && !/sticky top-0 z-10 py-1 text-center/.test(radar))
...
check('room exits stay pinned above a complete clickable floor browser', /max-h-\[42%\][^"']*overflow-y-auto/.test(classicRoom) && /aria-label="Room exits"/.test(classicRoom) && /aria-label="Clickable room items"/.test(classicRoom) && classicRoom.indexOf('aria-label="Room exits"') < classicRoom.indexOf('<FloorItems items={items}'))
...
check('radar rails reach the true bottom edge - nothing reserves floor-glance space any more', /style=\{\{ width, bottom: 0 \}\}/.test(radar) && !/--radar-loot-height/.test(radar))
check('the one floor browser is a controlled selection, settable from its own room-text parent', /selectedItem=\{floorSelection\}/.test(battle) && /onSelectedItemChange=\{setFloorSelection\}/.test(battle) && /selectedItem === undefined/.test(floor) && !/mode/.test(floor.match(/export function FloorItems\(\{[\s\S]*?\}\) \{/)?.[0] ?? ''))

...
const reels = JSON.parse(readFileSync('data/art/scene-reels.json', 'utf8'))
const reelFrames = reels.reels.flatMap((reel) => reel.frames)
check('every runtime reel is explicitly approved', reels.reels.every((reel) => reel.audit.status === 'visually-reviewed' && reel.audit.verdict === 'approved'))
check('every approved reel frame exists', reelFrames.every((frame) => existsSync(`public${frame.path}`)))
check('every reel frame carries timestamp and perceptual provenance', reelFrames.every((frame) => Number.isFinite(frame.timestampSeconds) && frame.perceptualHash))

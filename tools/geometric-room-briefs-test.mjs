...
if (!existsSync(outputPath)) fail('compiler writes its full generated catalogue')
  ...
  else pass('figurative customer traffic does not classify Town Green as water')
  if (/cute geometric|chunky stylized|5-metre geometric blocks/.test(crossingNorthRoom.prompt)) fail('superseded cartoon block direction leaked into production brief')
  else pass('current production briefs exclude superseded cartoon block direction')

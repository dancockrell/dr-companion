    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <header className="px-4 pt-4 pb-3 border-b border-border space-y-3 shrink-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <select
            ...
            value={uiMode}
            onChange={(e) => setUiMode(e.target.value as 'simple' | 'standard' | 'power')}
            title="UI complexity"
          >
      ...
      <section className="px-4 py-4 space-y-3 shrink-0">
        <Button
          ...
          icon={lowHealth ? <Heart className="w-5 h-5" /> : <Play className="w-5 h-5" />}
      ...
      {/* Inventory pressure (simple) */}
      {inventory && (
        <section className="px-4 pb-3 shrink-0">
          <div className="rounded-xl border border-border bg-surface-raised px-3 py-2 flex items-center justify-between text-sm">

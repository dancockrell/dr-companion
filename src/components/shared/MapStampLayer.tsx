...
export function MapStampLayer({
  ...
  return (
    ...
    >
      {stamps.map((stamp) => {
        const size = (stamp.kind === 'seal' ? 27 : 34) * unit * stamp.weight
        return (
          <g
            key={`${stamp.kind}-${stamp.x}-${stamp.y}`}
            transform={`translate(${xFor(stamp.x)} ${yFor(stamp.y)}) rotate(${stamp.rotation})`}
            opacity={stamp.kind === 'seal' ? 0.3 : 0.4}
          >
            <MapDrawing kind={stamp.kind} size={size} strokeWidth={Math.max(0.65, 0.9 * unit)} />
          </g>
        )
      })}
    </g>
...
function MapDrawing({ kind, size: s, strokeWidth: line }: { kind: MapStamp['kind']; size: number; strokeWidth: number }) {
  if (kind === 'water') {
  ...
  if (kind === 'settlement') {
    return <>
      <path d={`M ${-s * 0.68} ${s * 0.48} V ${-s * 0.06} L ${-s * 0.42} ${-s * 0.34} L ${-s * 0.14} ${-s * 0.06} V ${s * 0.48} M ${-s * 0.08} ${s * 0.48} V ${-s * 0.24} H ${s * 0.2} V ${s * 0.48} M ${s * 0.28} ${s * 0.48} V ${s * 0.02} L ${s * 0.48} ${-s * 0.2} L ${s * 0.68} ${s * 0.02} V ${s * 0.48} M ${-s * 0.76} ${s * 0.48} H ${s * 0.76}`} strokeWidth={line} />
      <path d={`M ${s * 0.06} ${-s * 0.24} V ${-s * 0.56} M ${-s * 0.02} ${-s * 0.46} H ${s * 0.14}`} strokeWidth={line * 0.76} />
    </>
  }
  if (kind === 'burial') {
    return <>

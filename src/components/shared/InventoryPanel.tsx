  ...
  return (
    <div className="group flex min-w-0 items-center gap-1 border-t border-border/50 px-2 py-1 first:border-t-0">
      ...
      <div className="flex shrink-0 items-center opacity-60 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
        <button type="button" className="rounded px-1 py-0.5 text-ink-faint hover:bg-surface-overlay hover:text-ink" onClick={() => requestGameAction(`appraise ${target} quick`, `Appraise ${name}`)} title="Quick appraisal" aria-label={`Quick-appraise ${name}`}><Scale className="h-3.5 w-3.5" /></button>
        <button type="button" className="rounded px-1 py-0.5 text-ink-faint hover:bg-surface-overlay hover:text-ink" onClick={() => requestGameAction(`analyze ${target}`, `Analyze ${name}`)} title="Analyze crafting and special properties"><Sparkles className="h-3.5 w-3.5" /></button>
        <button type="button" className="rounded px-1 py-0.5 text-ink-faint hover:bg-surface-overlay hover:text-info" onClick={() => onWiki(name)} title={`Look up ${name} on Elanthipedia`} aria-label={`Elanthipedia information for ${name}`}><BookOpen className="h-3.5 w-3.5" /></button>
      </div>
    </div>
  )
}

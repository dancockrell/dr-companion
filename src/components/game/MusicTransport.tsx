...
import { refreshMusicLibrary } from '../../lib/musicLibrary.ts'
import { MusicInstallAction } from './MusicInstall.tsx'
  ...
  return (
    <div className={cn('flex min-w-0 flex-col gap-1', className)}>
      <div className="flex min-w-0 items-center gap-0.5">
        ...
        {/* Every action in the unavailable state is an install, and none of
          * them is a Retry: `musicRetryable` is false here (see
          * ambientSound.ts), so the two are mutually exclusive by construction
          * rather than by both being asked to behave. `groupId` is the group
          * the track that could not play belongs to, so what is offered first
          * is the smallest thing that would fix what the listener just
          * noticed - see MusicInstall.tsx. */}
        {unavailable && <MusicInstallAction groupId={now?.groupId} />}
        {retryable && (
          <button
            type="button"
            onClick={retryMusic}

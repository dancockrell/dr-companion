...
export function classifyTether(command, direction) {
  ...
  if (/\b(stairs?|stairways?|staircases?|steps|up|down)\b/.test(text)) return 'stairs'
  if (/\b(doors?|doorways?|backdoors?|trapdoors?|gates?|gateways?|arches|archways?|arch|curtains?|thresholds?|openings?|entrances?|exits?|out)\b/.test(text)) return 'threshold'

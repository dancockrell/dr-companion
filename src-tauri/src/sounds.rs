...
/// The picker's filter stood here until N6.
///
/// `is_audio_filename` existed for `list_sounds`, which fed the sound picker in
/// the Genie highlights editor; the editor is gone, `list_sounds` went with it,
/// and a filter with no picker to filter for is a function clippy correctly
/// calls dead. `read_sound` keeps its own extension check inline - the two were
/// never identical, because a leading-dot backup file must stay out of a picker
/// and must still resolve when a config names it by hand.
#[tauri::command]

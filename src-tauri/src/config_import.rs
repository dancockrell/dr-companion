...
//!
//! # Read-only again, as of 6 Sep 2026
//!
//! This module said flatly, until 29 Aug 2026, that it never writes into a
//! Genie install: those files belong to a program the player may still be
//! using, and a client that edits another client's config is one bad release
//! away from destroying settings it did not create. `write_genie_config`
//! reversed that for the in-app highlights/aliases editor, N6 deleted the
//! editor, and Q5 deleted the write path with `docs/PLAYER_CONFIG.md` §8's
//! answer to question N-c: the player's own files are app data, and they live
//! in `app_data_dir()/config` through `player_files.rs`, which is where
//! `sibling`, `backup_once`, `save_atomically` and `matches_on_disk` moved
//! along with their tests.
//!
//! So the property is restored rather than merely stated: **this module
//! reads, and nothing in this app writes into a Genie install.**
//! `tools/doc-claims-test.mjs` asserts it, with a sabotage that reddens when a
//! second module reaches for a Genie writer.
//!
//! `player_files::adopt_genie_file` calls `read_genie_config` below to copy a
//! pre-Q5 pin file into app data once. That is this module's read, used in the
//! one direction it has ever been safe to use.

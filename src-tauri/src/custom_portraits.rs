...
fn path(name: &str, instance: &str) -> PathBuf {
    crate::setup::app_data_dir().join("portraits").join(format!("{}.webp", key(name, instance)))
}

fn dimensions(bytes: &[u8]) -> Option<(u16, u16)> {
    if bytes.len() < 16 || &bytes[0..4] != b"RIFF" || &bytes[8..12] != b"WEBP" { return None; }
    bytes.windows(3).position(|part| part == [0x9d, 0x01, 0x2a]).and_then(|at| {
        if at + 7 > bytes.len() { return None; }
        Some((u16::from_le_bytes([bytes[at + 3], bytes[at + 4]]) & 0x3fff,
              u16::from_le_bytes([bytes[at + 5], bytes[at + 6]]) & 0x3fff))
    })
}

fn validate(bytes: &[u8]) -> Result<(), String> {
    if bytes.is_empty() || bytes.len() > MAX_BYTES { return Err("Portrait must be a non-empty WebP no larger than 1 MB.".into()); }
    let (width, height) = dimensions(bytes).ok_or("Portrait is not a valid lossy WebP image.")?;
    if width == 0 || height == 0 || width > MAX_DIMENSION || height > MAX_DIMENSION { return Err("Portrait dimensions are invalid or exceed 2048 pixels.".into()); }
    Ok(())
}

#[tauri::command]
pub fn save_custom_portrait(name: String, instance: String, webp_base64: String) -> Result<String, String> {
    let bytes = STANDARD.decode(webp_base64).map_err(|_| "Portrait data is not valid base64.".to_string())?;
    validate(&bytes)?;
...
pub fn read_custom_portrait(name: String, instance: String) -> Result<Option<String>, String> {
    ...
    if validate(&bytes).is_err() { return Ok(None); }
    Ok(Some(format!("data:image/webp;base64,{}", STANDARD.encode(bytes))))
}
...
mod tests {
    use super::*;
    #[test] fn identity_is_instance_scoped() { assert_ne!(key("Ari", "Prime"), key("Ari", "Platinum")); }
    #[test] fn corrupt_input_is_rejected() { assert!(validate(b"not an image").is_err()); }
    #[test] fn excessive_dimensions_are_rejected() {
        let mut bytes = b"RIFF0000WEBPVP8 0000\x9d\x01\x2a".to_vec();
        bytes.extend_from_slice(&3000u16.to_le_bytes()); bytes.extend_from_slice(&1024u16.to_le_bytes());

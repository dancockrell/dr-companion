...
fn main() {
    ...
    rt.block_on(async {
        ...
        println!(
            "    EXPTracker.dll present: {}",
            dir.join("EXPTracker.dll").exists()
        );
        ...
        match install_bundle_inner(&tampered, &bad_dir.to_string_lossy(), |_, _| {}).await {
            Ok(_) => {
                println!("FAIL: accepted a bad hash");
                std::process::exit(1);
            }
        ...
        if leaked > 0 {
            println!("FAIL: wrote a file that did not verify");
            std::process::exit(1);
        }

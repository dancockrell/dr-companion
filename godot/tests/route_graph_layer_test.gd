...
func _initialize() -> void:
	...
	layer.render_routes(typed_cells, "b")
	_ok("room focus excludes unrelated connections", layer.segment_count() == 1 and layer.segment_count_for("road") == 1)
	layer.render_routes(typed_cells)
	_ok("world framing restores the complete graph", layer.segment_count() == 2)
	layer.free()
